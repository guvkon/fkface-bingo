...
var $author$project$Main$Model = F3(
	function (key, url, board) {
		return {r: board, H: key, aj: url};
	});
...
var $author$project$Main$urlToBoard = function (maybeUrl) {
	if (!maybeUrl.$) {
		var url = maybeUrl.a;
		var _v1 = $author$project$Main$toRoute(url);
		switch (_v1.$) {
			case 2:
				return $elm$core$Maybe$Nothing;
			case 0:
				return $elm$core$Maybe$Nothing;
			default:
				var encodedString = _v1.a;
				return $author$project$Main$decodeBoard(encodedString);
		}
	} else {
		return $elm$core$Maybe$Nothing;
	}
	...
	function (_v0, url, key) {
		return _Utils_Tuple2(
			A3(
				...
				key,
				url,
				$author$project$Main$urlToBoard(
...
var $author$project$Main$choices = _List_fromArray(
	['Night time shades', 'Double fisting', 'Separated at the crosswalk', 'Dumb hat', 'Looser board', 'Skate board', 'Fanny pack', 'Matching outfit', 'Guy on guy scooter', 'Dead Parrot Head', 'Crosswalk coward', 'Shirtless dude', 'Memaw', 'Kid out too late', 'Ghost tour bus / Street train', 'Trycicle', 'Pink taxi', 'Bouncer bounces', 'Drop something', 'Drop/spill food/drink', 'Litterbug', 'Jeans in flip-flops', 'Mullet', 'Bachelorette party', 'Go cart', 'Luxury car', 'Drunk walk', 'Wheel chair', 'Camera shy', 'Camera acknowledge', 'Bandage dude', 'Pickup / Dropoff', 'Dancing', 'Twerking', 'Sitting on the street', 'Chug', 'Aggressive honking', 'Trip', 'Someone yells for someone to wait up', 'Nose pick', 'Beach goth', 'Yawning', 'Trying to get into a wrong car', 'Formal attire', 'Hoofin it', 'Offensive T-shirt', 'Handshake', 'Couple fighting / regular fight', 'Cop car / ambulance / fire truck']);
...
var $author$project$Main$clickCell = F2(
	function (board, clickedOn) {
		var toggleCell = F2(
			function (position, _v0) {
				var value = _v0.a;
				var state = _v0.b;
				return _Utils_eq(position, clickedOn) ? _Utils_Tuple2(value, !state) : _Utils_Tuple2(value, state);
			});
		return A2($elm$core$List$indexedMap, toggleCell, board);
	});
...
var $author$project$Main$makeBoard = function (values) {
	return A3(
		$elm$core$List$map2,
		$elm$core$Tuple$pair,
		$elm$core$List$concat(
			_List_fromArray(
				[
					A2($elm$core$List$take, 12, values),
					_List_fromArray(
					['Free Space']),
				...
				if (!urlRequest.$) {
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								r: $author$project$Main$urlToBoard(
									$elm$core$Maybe$Just(url))
							}),
						A2(
			...
			case 2:
				return _Utils_Tuple2(
					...
					A2(
						...
						A2($elm_community$random_extra$Random$List$choices, 24, $author$project$Main$choices)));
			case 3:
				var _v2 = msg.a;
				var selected = _v2.a;
				var _v3 = $author$project$Main$encodeBoard(
					$author$project$Main$makeBoard(selected));
				if (!_v3.$) {
					var encodedBoard = _v3.a;
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								r: $author$project$Main$urlToBoard(
									$elm$url$Url$fromString('/board/' + encodedBoard))
							}),
						A2($elm$browser$Browser$Navigation$pushUrl, model.H, '/board/' + encodedBoard));
				} else {
						...
						model,
						{
							r: function () {
								...
								if (_v4.$ === 1) {
									...
									var board = _v4.a;
									return $elm$core$Maybe$Just(
										A2($author$project$Main$clickCell, board, position));
								}
							}()
						}),
					$elm$core$Platform$Cmd$none);
...
var $author$project$Main$viewBoardLink = function (board) {
	var maybeEncodedBoard = $author$project$Main$encodeBoard(board);
	if (!maybeEncodedBoard.$) {
		var encodedBoard = maybeEncodedBoard.a;
		return A2(
			$elm$html$Html$a,
			_List_fromArray(
				[
					$elm$html$Html$Attributes$href('/board/' + encodedBoard)
				]),
			_List_fromArray(
				[
					$elm$html$Html$text(
					A3($elm$core$String$slice, 0, 32, encodedBoard))
				]));
	} else {
		return $elm$html$Html$text('');
	}
};
var $author$project$Main$generateButton = function (maybeBoard) {
	if (maybeBoard.$ === 1) {
		return A2(
			$elm$html$Html$button,
			_List_fromArray(
				[
					$elm$html$Html$Attributes$class('btn btn-primary my-3'),
					$elm$html$Html$Events$onClick($author$project$Main$GenerateBoard)
				]),
			_List_fromArray(
				[
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							$elm$html$Html$text('Board link: '),
							$author$project$Main$viewBoardLink(board)
						])),
					A2(
					$elm$html$Html$button,
					_List_fromArray(
					...
					_List_fromArray(
						[
							$elm$html$Html$text('Re-Generate Board')
						]))
				]));
	}
};
							...
							[
								...
								_List_fromArray(
									[
										...
										_List_fromArray(
											[
												$elm$html$Html$Attributes$href('https://www.youtube.com/watch?v=kxsJ4PW_R04'),
								...
								_List_fromArray(
									[
										...
										_List_fromArray(
											[
												$elm$html$Html$text('Sloppy Joe\'s Key West Street cam -> ')
											])),
										A2(
										$elm$html$Html$a,
										_List_fromArray(
											[
												$elm$html$Html$Attributes$href('https://liveduvalstreet.com/'),
												$elm$html$Html$Attributes$target('_blank')
											]),
										_List_fromArray(
											[
												$elm$html$Html$text('[stream]')
											]))
									]))
							])),
...
var $author$project$Main$view = function (model) {
	return $author$project$Main$viewHome(model.r);
};
var $author$project$Main$main = $elm$browser$Browser$application(
	{ax: $author$project$Main$init, az: $author$project$Main$UrlChanged, aA: $author$project$Main$LinkClicked, aD: $author$project$Main$subscriptions, aF: $author$project$Main$update, aG: $author$project$Main$view});
